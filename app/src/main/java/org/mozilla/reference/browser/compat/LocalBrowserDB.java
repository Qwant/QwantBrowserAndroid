...
import org.mozilla.reference.browser.compat.BrowserContract.Favicons;
import org.mozilla.reference.browser.compat.BrowserContract.History;
import org.mozilla.reference.browser.compat.BrowserContract.PageMetadata;
import org.mozilla.reference.browser.compat.BrowserContract.SyncColumns;
...
public class LocalBrowserDB extends BrowserDB {
    ...
    private static ContentValues createBookmark(final long timestamp, final String title, final String url, final int pos, final long parent) {
        ...
        v.put(BrowserContract.Bookmarks.DATE_CREATED, timestamp);
        v.put(BrowserContract.Bookmarks.DATE_MODIFIED, timestamp);
        v.put(BrowserContract.Bookmarks.GUID, "guid");

        v.put(BrowserContract.Bookmarks.PARENT, parent);
        v.put(BrowserContract.Bookmarks.POSITION, pos);
        v.put(BrowserContract.Bookmarks.TITLE, title);
        v.put(BrowserContract.Bookmarks.URL, url);
        return v;
    }

    private static ContentValues createFavicon(final String url, final byte[] icon) {
        ContentValues iconValues = new ContentValues();
        iconValues.put(BrowserContract.Favicons.PAGE_URL, url);
        iconValues.put(BrowserContract.Favicons.DATA, icon);
    ...
    public boolean insertPageMetadata(ContentProviderClient contentProviderClient, String pageUrl, boolean hasImage, String metadataJSON) {
        ...
        cv.put(BrowserContract.PageMetadata.HISTORY_GUID, historyGUID);
        cv.put(BrowserContract.PageMetadata.HAS_IMAGE, hasImage);
        cv.put(BrowserContract.PageMetadata.JSON, metadataJSON);
    ...
    public int deletePageMetadata(ContentProviderClient contentProviderClient, String pageUrl) {
        ...
        try {
            return contentProviderClient.delete(mPageMetadataWithProfile, BrowserContract.PageMetadata.HISTORY_GUID + " = ?", new String[]{historyGUID});
        } catch (RemoteException e) {
            throw new IllegalStateException("Unexpected RemoteException", e);
        }
    }
    ...
    public Cursor getAllVisitedHistory(ContentResolver cr) {
        return cr.query(mHistoryUriWithProfile,
                        new String[] { History.URL, History.TITLE, History.DATE_LAST_VISITED },
                        History.VISITS + " > 0",
                        null,
                        null);
    }
    ...
    public Cursor getBookmarksInFolder(ContentResolver cr, long folderId) {
        ...
        if (folderId == Bookmarks.FAKE_DESKTOP_FOLDER_ID) {
            ...
            c = getReadingListBookmarks(cr); // getUrlAnnotations().getScreenshots(cr);
