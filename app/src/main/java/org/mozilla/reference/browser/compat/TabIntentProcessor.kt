...
class TabIntentProcessor(
        ...
        private val newTabSearchUseCase: QwantSearchUseCases.NewTabSearchUseCase,
    ...
    private fun processSendIntent(intent: SafeIntent): Boolean {
        ...
        return if (extraText.isNullOrBlank()) {
            ...
            val source = SessionState.Source.ACTION_SEND //  SessionState.Source.External.ActionSend(intent.externalPackage())
            if (url != null) {
    ...
    private fun processSearchIntent(intent: SafeIntent): Boolean {
        ...
        return if (searchQuery.isNullOrBlank()) {
            ...
            val source = SessionState.Source.ACTION_SEARCH // .External.ActionSearch(intent.externalPackage())
    ...
    private fun addNewTab(url: String, source: SessionState.Source) {
        if (isPrivate) tabsUseCases.addPrivateTab(url, source = source, flags = LoadUrlFlags.external())
        else tabsUseCases.addTab(url, source = source, flags = LoadUrlFlags.external())
    }
